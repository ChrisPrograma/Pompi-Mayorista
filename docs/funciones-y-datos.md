...
- **Mis productos**: precio, costo, ganancia por unidad y porcentaje. La ficha muestra el
  historial de precios anteriores con fecha. **Alta, edición y baja de productos**, y
  **cambio de precio a mano** desde la ficha.
- **Lo que llevo en el auto**: faltante calculado, botón para cargar todo lo que falta, y
  ajuste producto por producto.
- **Mis proveedores**: cuánto se le debe a cada uno y el total. **Alta, edición y baja**, con
  teléfono de contacto.
- **Mis clientes**: listado con zona, día de visita, contacto y deuda. **Alta, edición y baja**,
  y desde la ficha se registra el cobro.

### Agregar, editar y eliminar — cómo funciona cada uno

Las tres operaciones existen para productos, comercios y proveedores. Dos aclaraciones que
conviene tener a mano en la reunión, porque parecen limitaciones y son lo contrario:

**Editar no borra lo que no se muestra.** La pantalla manda solo los campos que tocó. Corregir el
teléfono de un comercio no le borra la zona ni el día de visita, aunque esos campos no estuvieran
a la vista. Un campo que se deja vacío a propósito sí se borra: es la forma de sacar un dato mal
cargado.

**Eliminar archiva, no destruye.** "Ya no le vendo más" saca al comercio de la lista de venta y
de la ruta; "Dejar de vender este producto" lo saca de la venta y de la carga del auto. En los
dos casos la ficha sigue existiendo y se puede reactivar.

| Si se borrara de verdad | Lo que pasaría |
|---|---|
| Un comercio con ventas | Las ventas quedarían apuntando a un comercio inexistente y la deuda de la calle dejaría de cerrar |
| Un producto vendido | Las ventas viejas perderían el nombre de lo que vendió, y la ganancia del mes se rompería |
| Un producto con stock | La mercadería real seguiría en la casa o en el auto, pero el sistema no la vería |

Ningún sistema de gestión serio borra estas filas. Archivar es lo que hacen todos, por esta razón.

**El precio y el stock no se editan desde la ficha del producto.** Cada uno tiene su propio
camino: el precio se cambia con "Cambiar el precio" (que apila, ver Parte 2) y el stock se mueve
entrando mercadería o cargando el auto. Si se pudieran escribir a mano en la ficha, se perdería
el historial que hace que el sistema sirva.
...
Un botón arriba de "Hoy" — **"Ver cómo funciona, paso a paso"** — arranca un recorrido de 15
pasos. Navega solo por las pantallas, resalta de qué está hablando y explica cada cosa en una
tarjeta abajo, sin tapar lo que se está mostrando. Se sale cuando uno quiera.

Está pensado para dos momentos: mostrarle la app al cliente, y que él mismo la vuelva a recorrer
la primera semana sin tener que preguntar.
...
| Venta por bulto | El producto se carga por unidad. Si compra por bulto y vende suelto hace falta un factor de conversión (pregunta 2 del martes) |
...
### Edición de un cliente, producto o proveedor

| | |
|---|---|
| **Escribe** | Reemplaza esa fila del catálogo, por id |
| **No toca** | Ventas, compras, movimientos de stock, precios ni pagos. Ninguno de ellos guarda el nombre: guardan el id |
| **Función del servidor** | `upsert` con `merge-duplicates` — la misma llamada que el alta |
| **Si se envía dos veces** | Queda igual: es la misma fila con el mismo id |

Ésta es la única parte del sistema donde se reemplaza una fila, y es deliberado: el nombre de un
comercio no es un hecho que pasó, es un dato que vale o no vale. Lo que **nunca** se reemplaza es
el libro mayor. Por eso corregir el nombre de un producto no cambia una sola cifra de las ventas
viejas: la venta guarda el `producto_id`, no el texto.

### Baja ("eliminar") de un cliente, producto o proveedor

| | |
|---|---|
| **Escribe** | La misma fila con `activo = false` |
| **No borra** | Nada. Ni la ficha, ni las ventas, ni el stock, ni la deuda |
| **Efecto** | Desaparece de las listas de venta, de la ruta y de la carga del auto |
| **Se puede deshacer** | Sí, volviendo a poner `activo = true` |

### Alta de un producto

| | |
|---|---|
| **Escribe** | 1 fila en `productos` · 1 fila en `precios_venta` con `vigente_hasta = NULL` |
| **Si se carga stock inicial** | Además 1 `compras` + 1 `compra_items` + 1 movimiento positivo en `deposito` |
| **El precio** | Nace como fila con vigencia, igual que todos los que vengan después |
| **El costo** | Sale de la compra. Sin carga inicial no hay costo, y la ganancia se ve inflada hasta la primera entrada de mercadería — la pantalla lo avisa |

La carga inicial se registra **contado**, no en cuenta: la mercadería que ya tenía en la casa no
es una deuda nueva con el proveedor. Marcarla en cuenta le inventaría una deuda que no existe.
...
Hay dos formas de llegar acá y **las dos hacen exactamente lo mismo**: aceptar una sugerencia
después de una compra más cara, o cambiarlo a mano desde la ficha del producto. No existe un
tercer camino que pise el precio, ni siquiera para corregir un error de tipeo: un precio mal
cargado se arregla cargando el correcto, y quedan los dos con sus fechas.
