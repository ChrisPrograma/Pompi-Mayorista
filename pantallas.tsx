...
export const PantallaHoy = ({ estado, hoy, acc }: Props) => {
  ...
  return (
    <div className="view">
      <button className="big-action" onClick={() => acc.ir('vender')}>
...
export const PantallaProveedores = ({ estado, acc }: Props) => {
  ...
  return (
    <div className="view">
      ...
      <div className="stack">
        {v.lista.map((p) => (
          <button className="row" key={p.id} onClick={() => acc.ir('ingreso')}>
            <span className="thumb"><Icono id="i-store" /></span>
    ...
    <div className="view">
      ...
      <div className="stack">
        {estado.clientes.filter((c) => c.activo).map((c) => {
          const d = deudas.lista.find((x) => x.clienteId === c.id);
          return (
            <button className="row" key={c.id} onClick={() => d && acc.cobrar(c.id)}>
              <span className="thumb"><Icono id="i-user" /></span>
              <span className="row-main">
                <b>{c.nombre}</b>
                <span>{c.zona}{c.diaVisita !== undefined ? ` · lo visitás los ${DIAS[c.diaVisita]}` : ''}</span>
              </span>
              <span className="row-end">
                {d ? <span className={`pill ${d.antiguedad === 'muy_atrasado' ? 'bad' : 'warn'}`}>
                  debe {plataCorta(d.saldoCent)}</span>
